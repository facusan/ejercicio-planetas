...
class Predictor:
    ...
    def predecir_clima_str(self, area, sol_dentro_recta, sol_dentro_triangulo):
        if area == 0 and sol_dentro_recta:
            return str(Clima.SEQUIA.value)
        if area == 0 and sol_dentro_recta == False:
            return str(Clima.CONDICIONES_OPTIMAS.value)
        if area == AREA_MAXIMA and sol_dentro_triangulo:
            return str(Clima.LLUVIA_INTESA.value)
        if area != 0 and sol_dentro_triangulo:
            return str(Clima.LLUVIA.value)
        return str(Clima.SIN_PRONOSTICO.value)
    ...
    def punto_dentro_de_recta(self, puntos_recta, punto):
        ...
        if((x2-x1 == 0 or x2 == punto_x)or(y2-y1 == 0 or y2== punto_y)): #Para evitar la division por 0
            return True
    ...
    def generar_predicciones(self,sistemaSolar, ndias):
        periodos = { str(Clima.SEQUIA.value) : 0, 
                    str(Clima.CONDICIONES_OPTIMAS.value): 0 , 
                    str(Clima.LLUVIA.value): 0, 
